...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="card">
            ...
            <div class="card-body">
                <a href="{{ route('admin.expense-claims.edit', $expenseClaim->id) }}" class="btn btn-warning w-100 mb-2">
                    <i class="bi bi-pencil"></i> Edit Claim
                </a>
                @if($expenseClaim->status !== 'reimbursed')
                    <form action="{{ route('admin.expense-claims.destroy', $expenseClaim->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
